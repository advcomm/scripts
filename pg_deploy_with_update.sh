...
# PostgreSQL deployment script with repository update capability
...
Usage: $0 [OPTIONS] <BACKUP_DIR> <REPO_PATH> <SSH_PATH> <DB_NAME> [DB_USER] [OS_USER_UID]

Required Arguments:
  BACKUP_DIR      Directory to store database backups
  REPO_PATH       Path to the Git repository
  SSH_PATH        Path to SSH private key for Git authentication
  DB_NAME         Database name to deploy

Optional Arguments:
  DB_USER         Database user (defaults to DB_NAME)
  OS_USER_UID     UID for OS user creation in container (defaults to 1001)
...
Description:
  ...
  If database doesn't exist:
    - Creates OS user in container for peer authentication
...
Examples:
  $0 /backup /repo ~/.ssh/id_rsa mydb
  $0 /backup /repo ~/.ssh/id_rsa mydb myuser 1001
  $0 --bypass-update /backup /repo ~/.ssh/id_rsa mydb myuser 1002
  $0 -b -c my-postgres /backup /repo ~/.ssh/id_rsa mydb myuser 1003
EOF
}
...
SSH_PATH="$3"
DATABASE_NAME="$4"
POSTGRES_USER="${5:-$DATABASE_NAME}"  # Use DB_NAME as default user if not provided
OS_USER_UID="${6:-10001}"  # Default UID 10001 if not provided
